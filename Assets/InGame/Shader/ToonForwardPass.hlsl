...
half3 GetShadowColor(half shadow, half material, half day)
{
    ...
    half3 shadowColor = shadowRamp * lerp(_ShadowColor, 1.0, smoothstep(0.9, 1.0, rampUV.x)).rgb;
...
half4 ForwardPassFragment(Varyings input, FRONT_FACE_TYPE facing : FRONT_FACE_SEMANTIC) : SV_TARGET
{
    ...
    half3 lightDirection = SafeNormalize(mainLight.direction.rgb * _LightDirectionMultiplier.rgb);
